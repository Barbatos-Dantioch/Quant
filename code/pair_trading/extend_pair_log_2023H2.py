...
# -*- coding: utf-8 -*-
"""
扩展 OU-P00-zs-cv5-h5 的 pair_log 到 2023H2
==========================================

为 XGBoost 实验补充训练数据: 生成 2023-07-01 ~ 2023-12-29 (约 122 个交易日) 的
pair_log, 流程与 OU-P00-zs-cv5-h5 完全一致 (zscore + cv5 + h5 + 行业内 cap=1).

输出: output/0506_ou_pair/_train_extra_2023H2/pair_log.parquet
      (独立目录, 不覆盖基线)
...
# Monkey-patch BACKTEST_START / END 让 prepare_data 加载到 2023H2 范围
import run_ou_pair as ROP
ROP.BACKTEST_START = "2023-07-01"
ROP.BACKTEST_END   = "2023-12-29"
# 价格表实际起于 2020-01-02; OU 训练窗 272 日历史 → DATA_START 需早于 2023-07-01 ~ 14 个月
# 设 2021-01-04 留出 ~600 交易日的安全 buffer (远超 272), 价格表完全支持
ROP.DATA_START     = "2021-01-04"

OUTPUT_DIR = "output/0506_ou_pair/_train_extra_2023H2"
...
def main():
    print(f"\n{'='*60}")
    print(f"扩展 pair_log (训练补充)  {time.strftime('%Y-%m-%d %H:%M:%S')}")
