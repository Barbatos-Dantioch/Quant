...
# -*- coding: utf-8 -*-
"""
OU 配对·LightGBM 信号实验 - expanding + 样本加权 (OU-P-RPL-LGBEXP-W-cv5-h5)
==========================================================================

与 OU-P-RPL-LGBEXP-cv5-h5 完全一致, 仅在训练时对样本按"实际收益"加权:
  单 pair 实际收益 r = sign(μ-X_t) · ΔX_{t→t+5}
  段内训练部分 (前 80%) 全局 rank 反向赋权: w = 2 - 2·rank_pct(r)
    → 收益最低 w≈2, 最高 w≈0, 均值≈1 (总有效样本量守恒)
  不做截面内归一化 (差时期样本普遍低收益 → 自动获更大总权重)
  val (后 20%) 不加权, 仅用于 early stopping
...
# ── 常量 ──
EXP_NAME = "OU-P-RPL-LGBEXP-W-cv5-h5"
...
def run_xgb_rolling(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[Dict], List[pd.DataFrame]]:
    ...
    for seg in ROLLING_SEGMENTS:
        ...
        # ── 样本加权: 单 pair 实际收益 r = sign(μ-X)·ΔX, 训练部分全局 rank 反向赋权 ──
        # 收益低 → 权重大 (w = 2 - 2·rank_pct, 均值≈1, 收益最低 w≈2, 最高 w≈0)
        # 不做截面归一化: 差时期样本普遍低收益, 在全窗口 rank 中靠后, 自动获更大总权重
        r_tr = (np.sign(train_seg.loc[m_tr, "mu"] - train_seg.loc[m_tr, "X_t"])
                * train_seg.loc[m_tr, "delta_X"])
        rank_pct_tr = r_tr.rank(pct=True, method="average")
        w_tr = (2.0 - 2.0 * rank_pct_tr).clip(lower=0.0).astype(np.float32).values
        print(f"    样本权重: 均值 {w_tr.mean():.3f}, "
              f"范围 [{w_tr.min():.3f}, {w_tr.max():.3f}]")
