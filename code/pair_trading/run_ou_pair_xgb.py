...
# -*- coding: utf-8 -*-
"""
OU 配对·XGBoost 信号实验 (OU-P-RPL-XGB-cv5-h5)
===============================================

复用 OU-P00-zs-cv5-h5 的 pair 池子, 用 XGBoost 预测未来 5 日 ΔX,
|预测 ΔX| 作为 top20 排序 magnitude, sign 沿用 sign(μ-X_t).

5 段半年 rolling 训练:
  段 1: train 2023-07-01 ~ 2023-12-26 → test 2024-01-02 ~ 2024-06-30
  段 2: train 2024-01-02 ~ 2024-06-25 → test 2024-07-01 ~ 2024-12-31
  段 3: train 2024-07-01 ~ 2024-12-25 → test 2025-01-02 ~ 2025-06-30
  段 4: train 2025-01-02 ~ 2025-06-25 → test 2025-07-01 ~ 2025-12-31
  段 5: train 2025-07-01 ~ 2025-12-25 → test 2026-01-02 ~ 2026-05-13
...
ROLLING_SEGMENTS = [
    ...
    ("seg2", "2024-01-02", "2024-06-25", "2024-07-01", "2024-12-31"),
    ("seg3", "2024-07-01", "2024-12-25", "2025-01-02", "2025-06-30"),
    ("seg4", "2025-01-02", "2025-06-25", "2025-07-01", "2025-12-31"),
    ("seg5", "2025-07-01", "2025-12-25", "2026-01-02", "2026-05-13"),
]
...
def run_xgb_rolling(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[Dict], List[pd.DataFrame]]:
    ...
    for seg in ROLLING_SEGMENTS:
        ...
        booster = xgb.train(
            ...
            evals=evals,
            early_stopping_rounds=XGB_EARLY_STOPPING if n_va > 0 else None,
            verbose_eval=False,
        )
        best_iter = booster.best_iteration if n_va > 0 else booster.num_boosted_rounds() - 1
        best_score = booster.best_score if n_va > 0 else None
        train_rmse = float(booster.predict(dtrain, iteration_range=(0, best_iter+1)).std())
        elapsed = time.time() - t0
        print(f"    XGB 训练完成, best_iter={best_iter}, "
        ...
        pred = booster.predict(dtest, iteration_range=(0, best_iter+1))
        ...
        train_logs.append({
            ...
            "best_val_rmse": float(best_score) if n_va > 0 else np.nan,
            "elapsed_sec": elapsed,
        })
        ...
        imp_df = pd.DataFrame([
            {"seg": seg_name, "feature": f, "gain": score_dict.get(f, 0.0)}
            for f in FEATURES
