...
# -*- coding: utf-8 -*-
"""
OU 配对·LightGBM 信号实验 - expanding + 样本加权 v2 (OU-P-RPL-LGBEXP-W2-cv5-h5)
============================================================================

在 W (v1) 基础上修正两点, 解决 v1 全段 best_iter=1 的训练退化问题:
  1. 权重映射改为 w = 2 - rank_pct(r), 范围 [1, 2] (只增不减, 不丢高收益样本信息)
  2. val 也用同样权重 (保持 train/val 优化目标一致, early stopping 才准)

加权逻辑:
  单 pair 实际收益 r = sign(μ-X_t) · ΔX_{t→t+5}
  全局 rank 反向赋权: w = 2 - rank_pct(r) → 收益最低 w≈2, 最高 w≈1
  不做截面内归一化 (差时期样本普遍低收益 → 自动获更大总权重)
  train 和 val 各自在自身样本内算 rank 赋权
...
# ── 常量 ──
EXP_NAME = "OU-P-RPL-LGBEXP-W2-cv5-h5"
...
def run_xgb_rolling(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[Dict], List[pd.DataFrame]]:
    ...
    for seg in ROLLING_SEGMENTS:
        ...
        # ── 样本加权 v2: 单 pair 实际收益 r = sign(μ-X)·ΔX, rank 反向赋权 ──
        # 收益低 → 权重大 (w = 2 - rank_pct, 范围 [1, 2], 只增不减, 不丢高收益样本)
        # 不做截面归一化: 差时期样本普遍低收益, rank 靠后, 自动获更大总权重
        # train / val 各自在自身样本内算 rank (val 也加权, 保持优化目标一致)
        def _calc_weight(mask):
            r = (np.sign(train_seg.loc[mask, "mu"] - train_seg.loc[mask, "X_t"])
                 * train_seg.loc[mask, "delta_X"])
            rank_pct = r.rank(pct=True, method="average")
            return (2.0 - rank_pct).astype(np.float32).values
        w_tr = _calc_weight(m_tr)
        w_va = _calc_weight(m_va) if n_va > 0 else None
        print(f"    样本权重: train 均值 {w_tr.mean():.3f} 范围 [{w_tr.min():.3f}, {w_tr.max():.3f}]")
