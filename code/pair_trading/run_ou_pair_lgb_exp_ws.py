...
# -*- coding: utf-8 -*-
"""
OU 配对·LightGBM 信号实验 - expanding + 截面收益加权 (OU-P-RPL-LGBEXP-WS-cv5-h5)
==============================================================================

与 W2 唯一区别: 收益从"单 pair"改为"截面均值" (按截面收益加权, 同日所有 pair 同权重).

加权逻辑:
  截面收益 r_section(date) = 该日所有 is_legal pair 的 sign(μ-X)·ΔX 均值
    (衡量当日 OU 回归整体好坏, 不依赖选股方法, 无循环依赖)
  在各日期上 rank 反向赋权: w(date) = 2 - rank_pct(r_section), 范围 [1, 2]
    → 截面收益最低的日子 w≈2, 最高 w≈1
  同一日期所有 pair 共享该权重 (截面级加权)
  train / val 各自在自身日期内算 rank 赋权 (val 也加权)

其它: 同样 28 特征, 5 段 expanding, train/val 80/20 + 5 日 gap,
完整训练池 pair_log_full, sign=sign(μ-X_t), magnitude=|预测 ΔX|, holding=5.

训练池: 直接读 pair_log_full.parquet (由 build_full_pair_log.py 预合并,
覆盖 2021-07-01~2026-05-13).

训练窗模式 (环境变量 ROLL_DAYS):
  0(默认) = expanding, 固定起点 TRAIN_START, 训练终点随段增长
  >0      = 固定滚动窗, 每段训练窗 = 该段 train_end 往前 ROLL_DAYS 个交易日 (忽略 TRAIN_START)
            如 ROLL_DAYS=504 ≈ 过去 2 年, 实验名加 -ROLL504 后缀

训练窗模式 (环境变量 ROLL_DAYS):
  0(默认) = expanding, 固定起点 TRAIN_START, 训练终点随段增长
  >0      = 固定滚动窗, 每段训练窗 = 该段 train_end 往前 ROLL_DAYS 个交易日 (忽略 TRAIN_START)
            例: ROLL_DAYS=504 ≈ 过去 2 年, 实验名 OU-P-RPL-LGBEXP-WS-ROLL504-cv5-h5

5 段 expanding 训练 (训练起点 = TRAIN_START, 默认 2023-07-01, 训练终点随段增长):
  段 1: train TRAIN_START ~ 2023-12-26 → test 2024-01-02 ~ 2024-06-30
  段 2: train TRAIN_START ~ 2024-06-25 → test 2024-07-01 ~ 2024-12-31
  段 3: train TRAIN_START ~ 2024-12-25 → test 2025-01-02 ~ 2025-06-30
  段 4: train TRAIN_START ~ 2025-06-25 → test 2025-07-01 ~ 2025-12-31
  段 5: train TRAIN_START ~ 2025-12-25 → test 2026-01-02 ~ 2026-05-13
...
# ── 常量 ──
# 随机种子: 默认 42 (原实验); 通过环境变量 LGB_SEED 切换, 种子≠42 时实验名加后缀
_SEED = int(os.environ.get("LGB_SEED", "42"))
# 训练起点: 环境变量 TRAIN_START 控制 (默认 2023-07-01, 与原实验一致)
# 完整训练池 pair_log_full.parquet 覆盖 2021-07-01~2026-05-13, 可取该范围内任意起点
# 非默认起点时实验名加 -TS{YYYYMM} 后缀, 避免覆盖原结果
_DEFAULT_TRAIN_START = "2023-07-01"
TRAIN_START = os.environ.get("TRAIN_START", _DEFAULT_TRAIN_START)
_TS_TAG = "" if TRAIN_START == _DEFAULT_TRAIN_START else f"-TS{pd.Timestamp(TRAIN_START):%Y%m}"
# 训练窗模式: 环境变量 ROLL_DAYS
#   0(默认)=expanding (固定起点 TRAIN_START, 训练终点随段增长)
#   >0     =固定滚动窗 (每段训练窗 = 该段 train_end 往前 ROLL_DAYS 个交易日, 忽略 TRAIN_START)
_ROLL_DAYS = int(os.environ.get("ROLL_DAYS", "0"))
if _ROLL_DAYS > 0:
    _WIN_TAG = f"-ROLL{_ROLL_DAYS}"   # rolling 模式下 TRAIN_START 无意义, 用窗长标识
else:
    _WIN_TAG = _TS_TAG
# 市场/行业因子消融: 环境变量 DROP_GROUPS
#   ""(默认)=全 28 特征; "mkt"=去 3 个市场截面(25); "mktind"=去市场+行业(22)
_DROP_GROUPS = os.environ.get("DROP_GROUPS", "").strip().lower()
assert _DROP_GROUPS in ("", "mkt", "mktind"), f"DROP_GROUPS 仅支持 ''/mkt/mktind, 实际 {_DROP_GROUPS!r}"
_DROP_TAG = {"": "", "mkt": "-noMkt", "mktind": "-noMktInd"}[_DROP_GROUPS]
# 多种子集成: 环境变量 ENSEMBLE_SEEDS (逗号分隔, 如 "42,33"); 段内各种子各训一模型, 推理取预测均值
#   不设(默认)=单种子模式, 沿用 LGB_SEED 原行为; 设了则忽略 LGB_SEED, 实验名加 -ens{N}
_ENS_RAW = os.environ.get("ENSEMBLE_SEEDS", "").strip()
if _ENS_RAW:
    ENSEMBLE_SEEDS = [int(s) for s in _ENS_RAW.split(",") if s.strip()]
    _ENS_TAG = f"-ens{len(ENSEMBLE_SEEDS)}"
else:
    ENSEMBLE_SEEDS = [_SEED]
    _ENS_TAG = ""
# 调参实验后缀: 环境变量 RUN_TAG (如 lr03/cpx4/reg5), 实验名加 -{RUN_TAG}, 便于单维调参区分
_RUN_RAW = os.environ.get("RUN_TAG", "").strip()
_RUN_TAG = f"-{_RUN_RAW}" if _RUN_RAW else ""
# 候选池来源: 环境变量 PAIR_POOL (如 NZCdedup); 设了则读 pair_log_full_{PAIR_POOL}.parquet
#   且实验名加 -{PAIR_POOL} 后缀; 缺省读基线 pair_log_full.parquet
_PAIR_POOL = os.environ.get("PAIR_POOL", "").strip()
_POOL_TAG = f"-{_PAIR_POOL}" if _PAIR_POOL else ""
if _ENS_TAG:
    EXP_NAME = f"OU-P-RPL-LGBEXP-WS{_DROP_TAG}{_WIN_TAG}{_POOL_TAG}{_ENS_TAG}{_RUN_TAG}-cv5-h5"
elif _SEED == 42:
    EXP_NAME = f"OU-P-RPL-LGBEXP-WS{_DROP_TAG}{_WIN_TAG}{_POOL_TAG}{_RUN_TAG}-cv5-h5"
else:
    EXP_NAME = f"OU-P-RPL-LGBEXP-WS{_DROP_TAG}{_WIN_TAG}{_POOL_TAG}{_RUN_TAG}-s{_SEED}-cv5-h5"
OUTPUT_DIR = "output/0506_ou_pair"
# 唯一训练池数据源 (输入, 始终在根目录); PAIR_POOL 设了则切到对应池文件
FULL_PAIR_LOG = os.path.join(
    OUTPUT_DIR, f"pair_log_full_{_PAIR_POOL}.parquet" if _PAIR_POOL else "pair_log_full.parquet")
# 实验输出子目录: 环境变量 OUT_SUBDIR (如 tuning_roll504), 把本批结果与 summary 单独归档; 缺省写在根目录
_OUT_SUBDIR = os.environ.get("OUT_SUBDIR", "").strip()
_EXP_ROOT = os.path.join(OUTPUT_DIR, _OUT_SUBDIR) if _OUT_SUBDIR else OUTPUT_DIR
EXP_DIR = os.path.join(_EXP_ROOT, EXP_NAME)
...
DATA_START = "2020-01-02"   # 数据最早日: 保证 2021-07 起信号日有 252 日历史算衍生特征
DATA_END = "2026-05-13"
_EPS = 1e-12

# 5 段 expanding 训练划分 (训练终点随段增长), 训练起点取 TRAIN_START
ROLLING_SEGMENTS = [
    # (段名, train_start, train_end_inclusive, test_start, test_end_inclusive)
    ("seg1", TRAIN_START, "2023-12-26", "2024-01-02", "2024-06-30"),
    ("seg2", TRAIN_START, "2024-06-25", "2024-07-01", "2024-12-31"),
    ("seg3", TRAIN_START, "2024-12-25", "2025-01-02", "2025-06-30"),
    ("seg4", TRAIN_START, "2025-06-25", "2025-07-01", "2025-12-31"),
    ("seg5", TRAIN_START, "2025-12-25", "2026-01-02", "2026-05-13"),
]
...
# ── LGB 超参单维调参: 环境变量覆盖 (缺省不覆盖, 对已跑实验零影响) ──
# 每个环境变量对应一个超参; 单维扫描时只设需要的几个, 配合 RUN_TAG 命名区分
def _env_num(key, cast):
    v = os.environ.get(key)
    return cast(v) if v not in (None, "") else None
for _k, _ek, _cast in [
    ("learning_rate",     "LGB_LR",         float),
    ("num_leaves",        "LGB_NUM_LEAVES", int),
    ("max_depth",         "LGB_MAX_DEPTH",  int),
    ("min_child_samples", "LGB_MIN_CHILD",  int),
    ("reg_lambda",        "LGB_REG_LAMBDA", float),
    ("feature_fraction",  "LGB_FEAT_FRAC",  float),
    ("bagging_fraction",  "LGB_BAG_FRAC",   float),
    ("num_threads",       "LGB_NUM_THREADS", int),   # 限制 LGB 线程, 避免与其它实验争抢核
]:
    _v = _env_num(_ek, _cast)
    if _v is not None:
        LGB_PARAMS[_k] = _v
_v = _env_num("LGB_N_ROUNDS", int)
if _v is not None:
    LGB_N_ROUNDS = _v
_v = _env_num("LGB_EARLY_STOP", int)
if _v is not None:
    LGB_EARLY_STOPPING = _v
...
# 消融: 按 DROP_GROUPS 剔除截面因子 (特征工程仍照常计算, 仅不进入训练/预测)
if _DROP_GROUPS in ("mkt", "mktind"):
    FEATURES = [f for f in FEATURES if f not in FEATURE_MARKET]
if _DROP_GROUPS == "mktind":
    FEATURES = [f for f in FEATURES if f not in FEATURE_INDUSTRY]
...
def load_all_pair_log() -> pd.DataFrame:
    """读取完整训练池 pair_log_full.parquet (2021-07-01~2026-05-13).

    该文件由 build_full_pair_log.py 预先合并 [基线 + 各扩展段] 生成,
    是唯一训练池数据源; 各实验按 TRAIN_START 等切片所需时间段.
    """
    print("读取完整训练池 pair_log ...")
    if not os.path.exists(FULL_PAIR_LOG):
        raise FileNotFoundError(
            f"完整训练池未生成: {FULL_PAIR_LOG}. 先跑 build_full_pair_log.py")
    pl_full = pd.read_parquet(FULL_PAIR_LOG)
...
def run_xgb_rolling(df: pd.DataFrame) -> Tuple[pd.DataFrame, List[Dict], List[pd.DataFrame]]:
    ...
    # 训练池所有交易日 (升序), 供 rolling 窗按交易日回溯 + 越界校验
    pool_dates = np.array(sorted(df_train_pool["date"].unique()))
    pool_min_date = pool_dates[0]
    # 校验: expanding 模式下 TRAIN_START 不得早于训练池最早日 (防止静默缩水训练窗)
    # rolling 模式下窗口自动 clamp 到池最早日, 无需校验
    if _ROLL_DAYS == 0 and pd.Timestamp(TRAIN_START) < pool_min_date:
        raise ValueError(
            ...
            f"请改用不早于该日的起点, 或先扩展 pair_log_full 覆盖更早区间")
    ...
    for seg in ROLLING_SEGMENTS:
        ...
        if _ROLL_DAYS > 0:
            # 滚动窗: 取 <= train_hi 的交易日里最近 ROLL_DAYS 个; 不足则 clamp 到池最早日
        ...
        # ── 截面收益加权: r_section = 该日所有 pair 的 sign(μ-X)·ΔX 均值 ──
        # 在各日期上 rank 反向赋权: w(date) = 2 - rank_pct(r_section), 范围 [1, 2]
        # 同日所有 pair 共享权重; 截面收益低的日子整体权重大
        # train / val 各自在自身日期内算 rank (val 也加权, 保持优化目标一致)
        def _calc_section_weight(mask):
            ...
            # 在日期维度 rank, 收益低 → 权重大
            date_rank = date_r.rank(pct=True, method="average")
            date_w = 2.0 - date_rank
            # 广播回每个 pair
            return sub["date"].map(date_w).astype(np.float32).values
        w_tr = _calc_section_weight(m_tr)
        w_va = _calc_section_weight(m_va) if n_va > 0 else None
        print(f"    截面权重: train 均值 {w_tr.mean():.3f} 范围 [{w_tr.min():.3f}, {w_tr.max():.3f}]")

        # Dataset 与种子无关, 各种子复用 (bagging 随机性由 params.random_state 控制)
        ...
        # ── 多种子集成: 各种子各训一模型, 累加 test 预测后取均值 ──
        t0 = time.time()
        n_seeds = len(ENSEMBLE_SEEDS)
        pred_sum = np.zeros(len(test_seg), dtype=np.float64)
        gain_sum = np.zeros(len(FEATURES), dtype=np.float64)
        for sd in ENSEMBLE_SEEDS:
            params = dict(LGB_PARAMS, random_state=sd)
            callbacks = []
            if n_va > 0:
            ...
            pred_sum += booster.predict(X_te, num_iteration=best_iter)
            gain_sum += booster.feature_importance(importance_type="gain")
            print(f"    [seed {sd}] best_iter={best_iter}"
                  + (f", val_rmse={best_score:.5f}" if (n_va > 0 and best_score is not None) else ""))
            train_logs.append({
        ...
        print(f"    {n_seeds} 种子集成完成, 耗时 {elapsed:.1f}s")

        # 特征重要性 (gain): 种子间均值
        gain_avg = gain_sum / n_seeds
        score_dict = dict(zip(FEATURES, gain_avg))
        imp_df = pd.DataFrame([
...
def main():
    ...
    save_summary_row(os.path.join(_EXP_ROOT, "summary.csv"), {
        "exp": EXP_NAME,
        "train_window": "",
